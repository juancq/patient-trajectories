...
class OptimizationConfig(JSONableMixin):
    ...
    lr_decay_power: float | None = 1.0 
    weight_decay: float = 0.01
    patience: int | None = None
    gradient_accumulation: int | None = None
    lr_scheduler: bool | None = True
    cosine_scheduler_cycles: int | None = 1
    ...
    def set_to_dataset(self, dataset: PytorchDataset):
        ...
        if self.lr_num_warmup_steps is None and self.lr_frac_warmup_steps is None:
            self.lr_num_warmup_steps = 0
            self.lr_frac_warmup_steps = 0
        elif self.lr_num_warmup_steps is None:

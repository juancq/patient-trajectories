...
class CustomEarlyStopping(EarlyStopping):
    def __init__(self, warmup, *args, **kwargs):
...
class ESTForGenerativeSequenceModelingLM(L.LightningModule):
        ...
        self.save_hyperparameters(
            {
                ...
                "metrics_config": dataclasses.asdict(metrics_config),
            }
        )
        self.build_metrics()
    ...
    def configure_optimizers(self):
        ...
        opt = schedulefree.AdamWScheduleFree(
            ...
            lr=self.optimization_config.init_lr,
            weight_decay=self.optimization_config.weight_decay,
            warmup_steps=self.optimization_config.lr_num_warmup_steps,
        )
        ...
        if self.optimization_config.lr_scheduler:
            ...
            scheduler = get_cosine_schedule_with_warmup(
                ...
                num_warmup_steps=self.optimization_config.lr_num_warmup_steps,
                num_training_steps=self.optimization_config.max_training_steps,
                num_cycles=self.optimization_config.cosine_scheduler_cycles,
            )
...
class PretrainConfig:
    ...
    load_from_model_dir: str | None = None
    # do I need this in addition to checkpoint? 
    ...
    logger_default_experiment_name: str = "generative_event_stream_transformer"

    wandb_logger_kwargs: None | dict[str, Any] = dataclasses.field(
        default_factory=lambda: {
            "name": "${logger_default_experiment_name}",
            "project": None,
            "team": None,
            "log_model": True,
            "do_log_graph": True,
    ...
    mlflow_logger: dict[str, Any] = dataclasses.field(
        default_factory=lambda: {
            "experiment_name": "${logger_default_experiment_name}",
            "run_name": None,
        }
    )
    ...
    checkpoint_config: dict[str, Any] = dataclasses.field(
        default_factory=lambda: {
...
def train(cfg: PretrainConfig):
    ...
    # Model
    if cfg.checkpoint:
        LM = ESTForGenerativeSequenceModelingLM.load_from_checkpoint(cfg.checkpoint)
    else:
    ...
    if optimization_config.patience is not None:
        callbacks.append(
            ...
            CustomEarlyStopping(warmup=5, monitor="tuning_loss", 
                            mode="min", patience=optimization_config.patience)
        )

    # this path is why loading from checkpoint leads to weird paths save paths
    checkpoint_callback = ModelCheckpoint(dirpath=cfg.save_dir / 'lightning_logs', 
            save_top_k=cfg.checkpoint_config.get('save_top_k'), monitor='tuning_loss', 
            filename='{epoch}-{step}-{tuning_loss:.2f}')
    callbacks.append(checkpoint_callback)

    # saves last epoch, to resume failed runs or continue unconverged runs
    checkpoint_callback = ModelCheckpoint(dirpath=cfg.save_dir / 'lightning_logs', 
            filename='last-{epoch}-{step}-{tuning_loss:.2f}')
    callbacks.append(checkpoint_callback)
    ...
    mlf_logger = MLFlowLogger(
        experiment_name=cfg.mlflow_logger['experiment_name'],
        run_name=cfg.mlflow_logger['run_name'],
        tracking_uri="file:./mlruns",
        save_dir=cfg.save_dir,
    )
    if trainer_kwargs.get('logger', None) is None:
        trainer_kwargs["logger"] = [tensor_logger, mlf_logger]
    else:
    ...
    #        #profiler='advanced'
    #        #PyTorchProfiler(filename='profiler_results',with_stack=False,
    #        
    #        #on_trace_ready=torch.profiler.tensorboard_trace_handler(cfg.save_dir / 'lightning_logs'),
    #        )
    trainer = L.Trainer(**trainer_kwargs)

    #lr_finder = L.pytorch.tuner.Tuner(trainer)
    #r = lr_finder.lr_find(LM, train_dataloaders=train_dataloader)
    #print(LM.lr)
    #return

    if cfg.checkpoint:
        logger.info(f'Restarting training from {cfg.checkpoint}')

    trainer.fit(model=LM, 
                train_dataloaders=train_dataloader, val_dataloaders=tuning_dataloader,
                ckpt_path=None)

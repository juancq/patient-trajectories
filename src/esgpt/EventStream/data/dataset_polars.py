...
class Dataset(DatasetBase[DF_T, INPUT_DF_T]):
    ...
    def _load_input_df(
        ...
        col_exprs = [pl.col(subject_id_col).alias("subject_id"), pl.col(subject_id_col)]
    ...
    def _agg_by_time(self):
        ...
        events_df = self.events_df.lazy()

        if self.config.agg_by_time_scale is None:
            logger.debug("Grouping into unique timestamps")
            grouped = events_df.group_by(["subject_id", "timestamp"], maintain_order=True)
        else:
            logger.debug("Aggregating timestamps into buckets")
            grouped = events_df.sort(["subject_id", "timestamp"], descending=False).group_by_dynamic(
                "timestamp",
        ...
        grouped = (
            ...
            .with_columns(
                ...
                .list.join(self.EVENTS_SEPARATOR)
                .cast(pl.Categorical)
                .alias("event_type"),
            )
        )
        ...
        self.events_df = grouped.drop("old_event_id").collect(streaming=True)
        ...
        self.dynamic_measurements_df = dynamic_measurements_df.collect(streaming=True)

...
class PytorchDataset(SeedableMixin, torch.utils.data.Dataset):
    ...
    * If a task dataframe name is specified in the configuration object, then there should be either a
    ...
    def filter_to_subset(self):
        ...
        orig_len = len(self)
        subject_ids_set = set(self.subject_ids)
        orig_n_subjects = len(subject_ids_set)
        rng = np.random.default_rng(self.config.train_subset_seed)
        subset_subjects = rng.choice(
            list(subject_ids_set),
            size=count_or_proportion(orig_n_subjects, self.config.train_subset_size),
            replace=False,
        )
        subset_subjects_set = set(subset_subjects)
        index_df = pl.DataFrame(self.index, schema=['subject_id', 'start', 'end'])
        index_df = index_df.with_row_index()
        index_df = index_df.filter(pl.col('subject_id').is_in(subset_subjects_set))
        valid_index = index_df.get_column('index').to_list()
        index = index_df.select(pl.col(['subject_id', 'start', 'end'])).to_numpy().tolist()
        self.index = list(map(tuple, index))
        self.labels = {t: [t_labels[i] for i in valid_index] for t, t_labels in self.labels.items()}
    ...
    def _seeded_getitem_works_downstream(self, idx: int) -> dict[str, list[float]]:
        """Returns a Returns a dictionary corresponding to a single subject's data.

        This function is a seedable version of `__getitem__`.

        ###
        This version works for downstream, but fails during pretraining. 
        ###
        """
        ...
        seq_len = (end - st) + 1 # +1 because end and st are actual indices
        #print(f'subject id {subject_id}, start: {st}, end: {end}, seqlen {seq_len}')
        if seq_len > self.max_seq_len:
            ...
            st += start_offset
            #print('#'*30)
            #print('end', end, 'st', st, 'max seq len', self.max_seq_len, '+', st+self.max_seq_len)
            end = min(end, st + self.max_seq_len-1)

        #if end+1 >self.max_seq_len:
        #    print('#'*30)
        ...
        #print('self.maxseqlen', self.max_seq_len)
        #print(seq_len)
        #print(st, end)
        #if seq_len >100:
        #    print('#############')
        ...
        #print(out["dynamic"].to_dense())
        #if end == st:
            #end += 1
        #    pass

        out["dynamic"] = out["dynamic"][st:end+1]
        #print(f'seq len {seq_len} start {st} end {end}')
        #print(out["dynamic"])
    ...
    def _seeded_getitem(self, idx: int) -> dict[str, list[float]]:
        ...
        seq_len = (end - st) # end and st are actual indices
        if seq_len > self.max_seq_len:
            match self.config.subsequence_sampling_strategy:
                case SubsequenceSamplingStrategy.RANDOM:
                    start_offset = np.random.choice(seq_len - self.max_seq_len)
            ...
            if st == end:
                end = st + 1 # ensure at least one event is selected
        elif st == end: # seq_len <= self.max_seq_len
            end = st + 1
        ...
        out["dynamic"] = JointNestedRaggedTensorDict.load_slice(self.NRTs_dir / f"{shard}.pt", subject_idx)
        out["dynamic"] = out["dynamic"][st:end]
